...
#elif (CORE_FREQ_RUN_IN_MHZ == 4)
                | SCG_RCCR_DIVCORE(1)       /* DIVCORE=2, Core clock= 4MHz*/
                | SCG_RCCR_DIVBUS(1)        /* DIVBUS=2, BUS= 2MHz*/
                | SCG_RCCR_DIVSLOW(3);      /* DIVSLOW=4,Flash clock=1MHz*/
#elif (CORE_FREQ_RUN_IN_MHZ == 1)
                | SCG_RCCR_DIVCORE(3)       /* DIVCORE=4, Core clock= 1MHz*/
                | SCG_RCCR_DIVBUS(0)        /* DIVBUS=1, BUS= 1MHz*/
                | SCG_RCCR_DIVSLOW(0);      /* DIVSLOW=1,Flash clock=1MHz*/
#else
...
#if ((CORE_FREQ_RUN_IN_MHZ == 8) || (CORE_FREQ_RUN_IN_MHZ == 4))
    /* SIRC_DIV2 set to 8MHz / 2 = 4MHz */
    SCG->SIRCDIV = SCG_SIRCDIV_SIRCDIV2(2);
#elif (CORE_FREQ_RUN_IN_MHZ == 1)
    /* SIRC_DIV2 set to 8MHz / 8 = 1MHz */
